...
import { Plus, Pin, PinOff, Trash2, Download, Bold, Italic, Search, FileText, Edit3, Loader2 } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function NotesApp() {
  ...
  const togglePin = (id: string) => {
    ...
    toast({
      ...
      description: note?.isPinned ? "Note removed from pinned notes." : "Note pinned to top.",
    })
  }
  ...
  const downloadAsPDF = async () => {
    ...
    try {
      ...
      // Add metadata
      pdf.setFontSize(10)
      pdf.text(`Created: ${selectedNote.createdAt.toLocaleDateString()}`, 20, pdf.internal.pageSize.height - 20)
      pdf.text(`Updated: ${selectedNote.updatedAt.toLocaleDateString()}`, 20, pdf.internal.pageSize.height - 10)
  ...
  return (
    <div className="flex h-screen bg-background">
      {/* Sidebar */}
      <div
        className={`${sidebarCollapsed ? "w-16" : "w-80"} border-r bg-muted/30 flex flex-col transition-all duration-300 ease-in-out`}
      >
        {!sidebarCollapsed && (
          <>
            {/* Header */}
            <div className="p-4 border-b">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button onClick={createNewNote} size="sm" className="hover:scale-105 transition-transform duration-200 cursor-pointer">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {sortedNotes.length === 0 ? (
                <div className="text-center text-muted-foreground mt-8 animate-fade-in">
                  ...
                  <p className="font-medium">{searchTerm ? "No notes found" : "No notes yet"}</p>
                  <p className="text-sm">
                  ...
                  {sortedNotes.map((note, index) => (
                    <Card
                      ...
                      className={`cursor-pointer transition-all duration-200 hover:bg-accent hover:shadow-sm hover:scale-[1.02] ${
                        selectedNote?.id === note.id ? "bg-accent border-primary shadow-sm scale-[1.02]" : ""
                      }`}
                      onClick={() => {
        ...
        {selectedNote ? (
          <>
            ...
            <div className="border-b p-4 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">Ctrl+N: New note • Ctrl+F: Search • Ctrl+S: Save</span>
              </div>
            </div>
          ...
          <div className="flex-1 flex items-center justify-center text-center animate-fade-in">
            <div>
              ...
              <Button onClick={createNewNote} className="hover:scale-105 transition-transform duration-200">
